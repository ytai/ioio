...
public class DigitalIOTest implements Test<Boolean> {
    ...
    public DigitalIOTest(IOIO ioio, ResourceAllocator alloc) throws InterruptedException {
        ioio_ = ioio;
        alloc_ = alloc;
    ...
    public Boolean run() throws ConnectionLostException, InterruptedException {
        Log.i("IOIOTortureTest", "Starting DigitalIOTest on pins: " + pin1_ + ", " + pin2_);
        try {
        ...
        Log.i("IOIOTortureTest", "Passed DigitalIOTest on pins: " + pin1_ + ", " + pin2_);
        return true;
    }

    private boolean runTest(int inPin, int outPin) throws InterruptedException,
        ...
        try {
            ...
            for (int i = 0; i < 10; ++i) {
                ...
                ioio_.sync();
                if (value != in.read()) {
                    Log.w("IOIOTortureTest", "Failed DigitalIOTest input: " + inPin + ", output: "
                            + outPin);
                    return false;
                }
                value = !value;
